...
def get_linear_lower_slope_relu(upper, lower, upper_g_, lower_g_, **kwargs):
    ...
    if isinstance(upper_g_, tf.Variable):
        ...
        upper_g = 0.*upper + upper_g_
    if isinstance(lower_g_, tf.Variable):
        lower_g = 0.*upper + lower_g_


    A = K.minimum(upper_g, 0.)
    B = K.maximum(lower_g, 0.)

    mask_zero = K.sign(K.maximum(upper,0.))
    mask_linear = K.sign(K.maximum(-lower, 0.))
    ...
    e_2_ += 10.*(1-mask_linear) + 10*(1-mask_zero)
