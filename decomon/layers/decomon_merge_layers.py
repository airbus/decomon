...
from tensorflow.keras.layers import Concatenate, Average, Maximum, Multiply, Minimum, Add, Subtract, Dot, Input
...
class DecomonAdd(Add, DecomonLayer):
    ...
    def call(self, inputs):
        ...
        inputs_x = inputs[1::n_comp]

        output_y = self.op(inputs_y)
        output_x = inputs_x[0]

        if self.mode == F_IBP.name:
            inputs_u = inputs[2::n_comp]
            inputs_l = inputs[3::n_comp]
        if self.mode == F_HYBRID.name:
            inputs_u = inputs[2::n_comp]
            inputs_w_u = inputs[3::n_comp]
            inputs_b_u = inputs[4::n_comp]
            inputs_l = inputs[5::n_comp]
            inputs_w_l = inputs[6::n_comp]
            inputs_b_l = inputs[7::n_comp]
        if self.mode == F_FORWARD.name:
            inputs_w_u = inputs[2::n_comp]
            inputs_b_u = inputs[3::n_comp]
            inputs_w_l = inputs[4::n_comp]
            inputs_b_l = inputs[5::n_comp]
...
def to_monotonic_merge(
    ...
    if mode == F_HYBRID.name:
        input_ = [
            [y_shape[i], x_shape, y_shape[i], w_shape[i], y_shape[i], y_shape[i], w_shape[i], y_shape[i]]
