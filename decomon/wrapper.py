...
def check_adv_box(model, x_min, x_max, source_labels, target_labels=None, batch_size=-1, fast=True):
    ...
    IBP = model_.IBP
    forward = model_.forward
    if not (forward or mode == "backward"):
    ...
    if batch_size > 0:
        ...
        def get_forward_sample(z_tensor, w_u, b_u, w_l, b_l, source_tensor, target_tensor=None, option="up"):
            ...
            argmax_ = (
                np.sign(np.maximum(w_u_f, 0.0)) * z_tensor[:, 1, :, None, None]
                + np.sign(np.minimum(w_u_f, 0.0)) * z_tensor[:, 0, :, None, None]
            )
            ...
            upper = np.reshape(upper, (upper.shape[0], -1))
            return argmax_[np.arange(len(argmax_)), :, np.argmax(upper, -1)]

        def get_forward_sample_up(z_tensor, w_u, b_u, w_l, b_l, source_tensor, target_tensor=None):
            return get_forward_sample(z_tensor, w_u, b_u, w_l, b_l, source_tensor, target_tensor, option="up")

        def get_forward_sample_low(z_tensor, w_u, b_u, w_l, b_l, source_tensor, target_tensor=None):
            return get_forward_sample(z_tensor, w_u, b_u, w_l, b_l, source_tensor, target_tensor, option="low")

        def get_backward_sample_up(z_tensor, w_u, b_u, w_l, b_l, y_tensor, target_tensor=None):
            return get_forward_sample_up(z_tensor, w_u[:, 0], b_u[:, 0], w_l[:, 0], b_l[:, 0], y_tensor, target_tensor)

        def get_backward_sample_low(z_tensor, w_u, b_u, w_l, b_l, y_tensor, target_tensor=None):
            return get_forward_sample_low(z_tensor, w_u[:, 0], b_u[:, 0], w_l[:, 0], b_l[:, 0], y_tensor, target_tensor)
        ...
        if forward:
            candidates_f_up = get_forward_sample_up(z, w_u_f, b_u_f, w_l_f, b_l_f, source_labels, target_labels)
            candidates_f_low = get_forward_sample_low(z, w_u_f, b_u_f, w_l_f, b_l_f, source_labels, target_labels)

        if mode == "backward":
            w_u_b, b_u_b, w_l_b, b_l_b = output[-4:]
            candidates_b_up = get_backward_sample_up(z, w_u_b, b_u_b, w_l_b, b_l_b, source_labels)
            candidates_b_low = get_backward_sample_low(z, w_u_b, b_u_b, w_l_b, b_l_b, source_labels)

    # reshape if needed
    if len(input_shape) > 1:
        if forward:
            candidates_f_up = np.reshape(candidates_f_up, [-1] + input_shape)
            candidates_f_low = np.reshape(candidates_f_low, [-1] + input_shape)

        if mode == "backward":
            candidates_b_up = np.reshape(candidates_b_up, [-1] + input_shape)
            candidates_b_low = np.reshape(candidates_b_low, [-1] + input_shape)

    if forward:
        adv_score_f = np.maximum(
            get_adv_box(
                model,
                candidates_f_up,
                candidates_f_up,
    ...
    if forward:
        if mode == "backward":
            adv_score = np.maximum(adv_score_f, adv_score_b)
        else:
            adv_score = adv_score_f
    else:
        adv_score = adv_score_b
...
def get_upper_box(model, x_min, x_max, batch_size=-1, n_sub_boxes=1, fast=True):
    ...
    if batch_size > 0:
        ...
        if mode == Backward.name:
            ...
            if len(w_u_b.shape) > 3:
                w_u_b = np.reshape(w_u_b, (-1, input_dim, shape_))
                b_u_b = np.reshape(b_u_b, (-1, shape_))
...
def get_lower_box(model, x_min, x_max, batch_size=-1, n_sub_boxes=1, fast=True):
    ...
    if batch_size > 0:
        ...
        if mode == Backward.name:
            ...
            w_l_b = w_l_b[:, 0]
            b_l_b = b_l_b[:, 0]
            if len(w_l_b.shape) > 3:
                w_l_b = np.reshape(w_l_b, (-1, input_dim, shape_))
                b_l_b = np.reshape(b_l_b, (-1, shape_))
...
def get_range_noise(model, x, eps, p=np.inf, batch_size=-1, fast=True):
    ...
    # check that the model is a DecomonModel, else do the conversion
    convex_domain = {"name": Ball.name, "p": p, "eps": max(0.0, eps)}
