...
class Helpers:
    ...
    def check_layer_linear(keras_layer, input_shape, method, decimal=6):
        ...
        keras_model = Helpers.build_keras_model(keras_layer, input_shape, input_dim, wo_linearity=True)
    ...
    def empirical_check_layer(keras_layer, input_shape, method, wo_linearity=False, decimal=6, keras_model=None):
        ...
        if keras_model is None:
            keras_model = Helpers.build_keras_model(keras_layer, input_shape, input_dim, wo_linearity=wo_linearity)
        else:
            input_dim = keras_model.inputs[0].shape[-1]
    ...
    def check_layer(
        ...
        if keras_model is None:
            ...
            input_dim = keras_model.inputs[0].shape[-1]
        torch_model = Helpers.build_torch_model(
            keras_layer, torch_layer, keras_model, input_shape, input_dim, wo_linearity=wo_linearity
        )

        Helpers.share_weights_torch_2_keras(torch_model, keras_model, axis_to_permute_kernel)
    ...
    def empirical_check_layer_backward_non_linear(keras_layer, input_shape, method, wo_linearity=False, decimal=6):
        ...
        keras_model = Helpers.build_keras_model(
            keras_layer, input_shape, input_dim, wo_linearity=wo_linearity, output_dim=1
        )
