...
def _test_backward_depthwise_conv2D_empirical(depth_multiplier, kernel_size, strides, padding, input_shape, method):
    keras_layer = DepthwiseConv2D(kernel_size=kernel_size, strides=strides, padding=padding, depth_multiplier=1)
...
def _test_backward_depthwise_conv1D_empirical(depth_multiplier, kernel_size, strides, padding, input_shape, method):
    keras_layer = DepthwiseConv1D(kernel_size=kernel_size, strides=strides, padding=padding, depth_multiplier=1)
    empirical_check_layer(keras_layer, input_shape, method=method, decimal=5)
...
def _test_backward_conv3d_empirical(filters, kernel_size, strides, padding, input_shape, method):
    # data_format == 'channels_first'
...
def _test_backward_conv2d_empirical(filters, kernel_size, strides, padding, input_shape, method):
    # data_format == 'channels_first'
...
def _test_backward_conv1d(filters, kernel_size, strides, padding, input_shape, method):
    ...
    check_layer(keras_layer, torch_layer, input_shape, axis_to_permute_kernel=(2, 1, 0), method=method, decimal=5)


def _test_backward_conv1d_empirical(filters, kernel_size, strides, padding, input_shape, method):
    # data_format == 'channels_first'
...
def test_backward_Conv2D_empirical(method, data_format):
    ...
    strides = 1
    padding = "same"
    _test_backward_conv2d_empirical(filters, kernel_size, strides, padding, input_shape, method)
...
def test_backward_Conv1D_empirical(method, data_format):
    keras.config.set_image_data_format(data_format)
    if data_format == "channels_first":
        input_shape = (2, 10)
    else:
    ...
    strides = 1
    padding = "same"
    _test_backward_conv1d_empirical(filters, kernel_size, strides, padding, input_shape, method)
...
@pytest.mark.parametrize("method", ["forward-ibp", "crown-forward-ibp", "crown"])
def test_backward_Conv3D_empirical(method):
    # skip tests on MPS device as Conv3DTranspose is not implemented
    if keras.config.backend() == "torch":
        import torch
    ...
    strides = 1
    padding = "same"
    _test_backward_conv3d_empirical(filters, kernel_size, strides, padding, input_shape, method)
...
def test_backward_DepthwiseConv2D(method, data_format):
    keras.config.set_image_data_format(data_format)
    if data_format == "channels_first":
        input_shape = (30, 10, 10)
    else:
    ...
    strides = 1
    padding = "same"
    _test_backward_depthwise_conv2D_empirical(depth_multiplier, kernel_size, strides, padding, input_shape, method)
...
def test_backward_DepthwiseConv1D_empirical(method, data_format):
    keras.config.set_image_data_format(data_format)
    if data_format == "channels_first":
        input_shape = (2, 10)
    else:
    ...
    depth_multiplier = 1
    padding = "same"
    _test_backward_depthwise_conv1D_empirical(depth_multiplier, kernel_size, strides, padding, input_shape, method)
