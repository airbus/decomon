...
def get_standart_values_1d_box(n, dc_decomp=True, grad_bounds=False, nb=100):
    ...
    if dc_decomp:
        ...
        return [x_[:,None], y_[:,None], x_0_[:,:, None],
                u_c_[:, None], w_u_[:, None, None], b_u_[:, None], l_c_[:, None], w_l_[:, None, None], b_l_[:, None],
                h_[:, None], g_[:, None]]
...
def get_tensor_decomposition_1d_box(dc_decomp=True):
    ...
    if dc_decomp:
        return [Input((1,)), Input((1,)), Input((2,1)), Input((1,)), Input((1,1)), Input((1,)),
                Input((1,)), Input((1, 1)), Input((1,)), Input((1,)), Input((1,)) ]
    return [Input((1,)), Input((1,)), Input((2, 1)), Input((1,)), Input((1, 1)), Input((1,)),
            Input((1,)), Input((1, 1)), Input((1,))]
...
def get_tensor_decomposition_multid_box(odd=1, dc_decomp=True):
    ...
    if dc_decomp:
        ...
        return [Input((n,)), Input((n,)), Input((2,n)), Input((n,)), Input((n,n)), Input((n,)), Input((n,)), Input((n,n)), Input((n,)),
                Input((n,)), Input((n,))]
    return [Input((n,)), Input((n,)), Input((2, n)), Input((n,)), Input((n, n)), Input((n,)), Input((n,)),
            Input((n, n)), Input((n,))]
...
def get_standard_values_multid_box(odd=1, dc_decomp=True):
    ...
    if dc_decomp:
        x_0, y_0, z_0, u_c_0, w_u_0, b_u_0, l_c_0, w_l_0, b_l_0, h_0, g_0 = get_standart_values_1d_box(0, dc_decomp)
        x_1, y_1, z_1, u_c_1, w_u_1, b_u_1, l_c_1, w_l_1, b_l_1, h_1, g_1 = get_standart_values_1d_box(1, dc_decomp)
    else:
        x_0, y_0, z_0, u_c_0, w_u_0, b_u_0, l_c_0, w_l_0, b_l_0 = get_standart_values_1d_box(0, dc_decomp)
        x_1, y_1, z_1, u_c_1, w_u_1, b_u_1, l_c_1, w_l_1, b_l_1 = get_standart_values_1d_box(1, dc_decomp)
    ...
    if not odd:
        ...
        x_2, y_2, z_2, u_c_2, w_u_2, b_u_2, l_c_2, w_l_2, b_l_2, h_2, g_2 = get_standart_values_1d_box(2)
...
def build_image_from_1D_box(odd=0, m=0, dc_decomp=True):
    ...
    if dc_decomp:
        x_, y_0, z_, \
        u_c_0, w_u_0, b_u_0, l_c_0, w_l_0, b_l_0, h_0, g_0 = get_standart_values_1d_box(m, dc_decomp=dc_decomp)
    else:
        x_, y_0, z_, \
        u_c_0, w_u_0, b_u_0, l_c_0, w_l_0, b_l_0 = get_standart_values_1d_box(m, dc_decomp=dc_decomp)
...
def build_image_from_2D_box(odd=0, m0=0, m1=1, dc_decomp=True):
    ...
    if dc_decomp:
        x_0, y_0, z_0, u_c_0, w_u_0, b_u_0, \
            l_c_0, w_l_0, b_l_0, h_0, g_0 = build_image_from_1D_box(odd, m0, dc_decomp)
        x_1, y_1, z_1, u_c_1, w_u_1, b_u_1,\
            l_c_1, w_l_1, b_l_1, h_1, g_1 = build_image_from_1D_box(odd, m1, dc_decomp)
    else:
        x_0, y_0, z_0, u_c_0, w_u_0, b_u_0, \
        l_c_0, w_l_0, b_l_0 = build_image_from_1D_box(odd, m0, dc_decomp)
        x_1, y_1, z_1, u_c_1, w_u_1, b_u_1, \
        l_c_1, w_l_1, b_l_1 = build_image_from_1D_box(odd, m1, dc_decomp)
    ...
    if dc_decomp:
        ...
        assert_output_properties_box(x_, h_ + g_, h_, g_, z_[:,0], z_[:,1], u_c_, w_u_, b_u_, l_c_, w_l_, b_l_, 'image from 2D')
...
def get_standard_values_images_box(data_format='channels_last', odd=0, m0=0, m1=1, dc_decomp=True):
    ...
    if data_format=='channels_last':
        ...
        if dc_decomp:
            ...
            assert_output_properties_box(x_, y_, h_, g_, z_min_, z_max_,\
                                        u_c_, w_u_, b_u_, l_c_, w_l_, b_l_, 'images {},{},{},{}'.format(data_format, odd, m0, m1))
        ...
        if dc_decomp:
            ...
            h_ = np.concatenate([h_0, h_0], 1)
            g_ = np.concatenate([g_0, g_0], 1)
            assert_output_properties_box(x_, y_, h_, g_, z_min_, z_max_, u_c_, w_u_, b_u_, l_c_, w_l_, b_l_, 'images {},{},{},{}'.format(data_format, odd, m0, m1))
...
def get_tensor_decomposition_images_box(data_format, odd, dc_decomp=True):
    ...
    if data_format=='channels_last':
        ...
        output = [Input((2,)), Input((n,n, 2)), Input((2,2)), Input((n, n, 2)), Input((2, n, n, 2)), Input((n, n, 2)),
                  Input((n, n, 2)), Input((2, n, n, 2)), Input((n, n, 2))]

        if dc_decomp:
            output+= [Input((n, n, 2)), Input((n, n, 2))]
    else:

        output = [Input((2,)), Input((2, n, n)), Input((2,2)), Input((2, n, n)), Input((2, 2, n, n)), Input((2, n, n)),
                  Input((2, n, n)), Input((2, 2, n, n)), Input((2, n, n))]
        if dc_decomp:
...
def assert_output_properties_box(x_, y_, h_, g_, x_min_, x_max_,u_c_, w_u_, b_u_, l_c_, w_l_, b_l_, name, decimal=6):
    ...
    assert_almost_equal( np.clip(x_min_-x_, 0, np.inf), 0., decimal=decimal, err_msg='x_min >x_  for function {}'.format(name))
    assert_almost_equal( np.clip(x_ - x_max_, 0, np.inf), 0., decimal=decimal, err_msg='x_max < x_  for function {}'.format(name))
