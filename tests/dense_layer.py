...
@pytest.mark.parametrize("n, activation", [(0, 'relu'), (1, 'relu'), (2, 'relu'), (3, 'relu'), (4, 'relu'), (5, 'relu'), (6, 'relu'), (7, 'relu'), (8, 'relu'), (9, 'relu'),
                                            (0, 'linear'), (1, 'linear'), (2, 'linear'), (3, 'linear'), (4, 'linear'), (5, 'linear'), (6, 'linear'), (7, 'linear'), (8, 'linear'), (9, 'linear'), \
...
def test_DecomonDense_1D_box(n, activation):
    ...
    assert_output_properties_box(x, y_ref, h_, g_, z_[:,0], z_[:, 1], u_c_, w_u_, b_u_, l_c_, w_l_, b_l_, 'dense_{}'.format(n))
...
@pytest.mark.parametrize("odd, activation",[(0, None), (1,None), (0,'linear'), (1, 'linear'), (0, 'relu'), (1, 'relu')] )
def test_DecomonDense_multiD_box(odd, activation):
    ...
    assert_output_properties_box(x, y_ref, h_, g_, z_[:,0], z_[:,1], u_c_, w_u_, b_u_, l_c_, w_l_, b_l_, 'dense_multid_{}'.format(odd))
...
def test_DecomonDense_1D_to_monotonic_box(n, activation):
    ...
    assert_almost_equal(np.minimum(W_pos,-W_neg), np.zeros_like(W_pos), decimal=6, err_msg='wrong decomposition')
    ...
    assert_output_properties_box(x, y_, h_, g_, z_[:,0], z_[:,1], u_c_, w_u_, b_u_, l_c_, w_l_, b_l_, 'dense_to_monotonic_{}'.format(n), decimal=5)
    assert np.allclose(y_ref, y_)
...
def test_DecomonDense_multiD_to_monotonic_box(odd, activation):
    ...
    assert_output_properties_box(x, y_, h_, g_, z_[:,0], z_[:, 1], u_c_, w_u_, b_u_, l_c_, w_l_, b_l_, 'dense_multid_to_monotonic{}'.format(odd))
...
def test_DecomonDense_1D_box_dc(n, activation):
