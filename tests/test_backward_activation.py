...
def test_activation_softsign_backward_1D_box(n, mode, previous, floatx):
    ...
    if mode == "hybrid":
        input_mode = inputs[2:]
        output = softsign(input_mode, dc_decomp=False, mode=mode)
        z_0, u_c_0, _, _, l_c_0, _, _ = output
    if mode == "forward":
        input_mode = [z, W_u, b_u, W_l, b_l]
        output = softsign(input_mode, dc_decomp=False, mode=mode)
    ...
    if previous:
        ...
        f_softsign = K.function(inputs + [w_out, b_out], [w_out_u, b_out_u, w_out_l, b_out_l])
        output_ = f_softsign(inputs_ + [np.ones((len(x_), 1, 1)), np.zeros((len(x_), 1))])
    else:
