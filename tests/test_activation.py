...
def test_softmax_1D_box(n, mode, floatx):
    ...
    if mode == "hybrid":
        output = softmax(inputs[2:], dc_decomp=False, mode=mode)
    if mode == "forward":
        output = softmax([z, W_u, b_u, W_l, b_l], dc_decomp=False, mode=mode)
    if mode == "ibp":
        output = softmax([u_c, l_c], dc_decomp=False, mode=mode)

    f_softmax = K.function(inputs[2:], output)
    f_ref = K.function(inputs, K.softmax(y))

    y_ = f_ref(inputs_)
    z_ = z_0

    if mode == "hybrid":
        z_, u_c_, w_u_, b_u_, l_c_, w_l_, b_l_ = f_softmax(inputs_[2:])
    if mode == "forward":
        z_, w_u_, b_u_, w_l_, b_l_ = f_softmax(inputs_[2:])
        u_c_, l_c_ = [None] * 2
    if mode == "ibp":
...
def test_exp_1D_box(n, mode, floatx):
    ...
    if mode == "hybrid":
        output = exponential(inputs[2:], dc_decomp=False, mode=mode)
    if mode == "forward":
        output = exponential([z, W_u, b_u, W_l, b_l], dc_decomp=False, mode=mode)
    if mode == "ibp":
        output = softmax([u_c, l_c], dc_decomp=False, mode=mode)
