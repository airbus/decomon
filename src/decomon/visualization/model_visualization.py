...
def get_layer_attribute(layer, name):
    ...
    for subname in subnames:
        attr = getattr(attr, subname, None)
        if attr is None:
            return None
    return attr
...
def make_layer_label(layer, **kwargs):
    ...
    for attr_name, attr_style in show_layer_attributes.items():
        attribute = get_layer_attribute(layer, attr_name)
        if attribute is not None:
            default_style = {"bgcolor": "white", "color": "black"}
            default_style.update(attr_style.get(None, {}))
            style_to_apply = dict(default_style)
            if attribute in attr_style:
                style_to_apply.update(attr_style[attribute])
            table += (
                f'<tr><td bgcolor="{style_to_apply["bgcolor"]}" colspan="{colspan}">'
                f'<font point-size="14" color="{style_to_apply["color"]}">'
                f"{attr_name.capitalize()}: <b>{attribute}</b>"
                "</font></td></tr>"
            )
    ...
    if show_shapes:
        ...
        try:
            ...
            pass
        cols.append(
            ('<td bgcolor="white"><font point-size="14">' f'Output shape: <b>{shape or "?"}</b>' "</font></td>")
        )
    if show_dtype:
        dtype = None
        try:
            ...
            pass
        cols.append(
            ('<td bgcolor="white"><font point-size="14">' f'Output dtype: <b>{dtype or "?"}</b>' "</font></td>")
        )
    if show_trainable and hasattr(layer, "trainable") and layer.weights:
        if layer.trainable:
            cols.append(
                ('<td bgcolor="forestgreen">' '<font point-size="14" color="white">' "<b>Trainable</b></font></td>")
            )
        else:
            cols.append(
                ('<td bgcolor="firebrick">' '<font point-size="14" color="white">' "<b>Non-trainable</b></font></td>")
            )
    if cols:
        colspan = len(cols)
    else:
