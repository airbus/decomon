...
def get_affine_upper_bound_max(lower:Tensor, upper:Tensor, axis:int, keepdims:bool=True)->Tuple[Tensor, Tensor]:
    ...
    corners_:Tensor = mask * l_reshaped + (o_value - mask) * (u_reshaped)  # (batch, shape_prev, n_dim, shape_after, n_dim)

    corners_collapse:Tensor = mask * l_reshaped + (o_value - mask) * (u_reshaped + index_collapse) # ?????? right shape but right values ?
    ...
    bias_corner:Tensor = o_value + K.sum(z_value * corners, axis_, keepdims=True) # (batch, shape_prev, 1, shape_after, n_dim+1)
    corners_collapse = K.concatenate([corners_collapse, bias_corner], axis=axis_)  # (batch, shape_prev, n_dim+1, shape_after, n_dim+1) 


    dimensions:array.array = np.arange(len(corners.shape)) # K.solve require that the matrix dimension is on the last two axis (-2, -1): A tensor of shape (..., M, M) representing the coefficients matrix. In our case M = n_dim+1

    dim_permutation:array.array = np.concatenate([dimensions[:axis_], dimensions[axis_ + 1 :], [dimensions[axis_]]])


    corners_collapse = K.transpose(corners_collapse, tuple(dim_permutation)) # (batch, shape_prev, shape_after, n_dim+1, n_dim+1)
    ...
    w_u, b_u = K.split(w_hull, [n_dim], axis=-1) # w_u : (None, shape_prev, shape_after, n_dim), b_u: (batch, shape_prev, shape_after, 1)
    b_u = K.reshape(b_u, [-1]+shape_prev+shape_after) # b_u (batch, shape_prev, shape_after) == lower.shape

    dim_permutation = np.concatenate([dimensions[:axis_], [len(lower.shape)-1], [e-1 for e in dimensions[axis_ + 1 :-1]]]).astype('int') # to check
    ...
    error:Tensor = K.maximum(z_value, K.cast(corners_pred,dtype=dtype)  - (K.sum(K.expand_dims(w_u,-1)*corners, axis_) + K.expand_dims(b_u, -1)))
    b_u += K.max(error, -1)
